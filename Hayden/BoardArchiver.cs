...
namespace Hayden
{
    ...
    public class BoardArchiver
    {
                ...
                case ResponseType.Ok:
                    ...
                    foreach (var thread in threadList)
                    {
                        if (thread.LastModified > lastCheckTimestamp)
                        {
                            ...
                            if (this.isWhitelisted(thread, boardConfig.Whitelist))
                            {
        ...
        private bool isWhitelisted(Post thread, string?[] whitelist)
        {
            if (whitelist != null)
            {
                foreach (var filter in whitelist)
                {
                    ...
                    {
                        // matched a whitelist regex
                        return true;
                    }
                }

                // didn't match any of the whitelist regexes
                return false;
            }
